...
- **MCP 运行时** -- 通过 HTTP/HTTPS 连接远程 MCP 服务器，支持 SSE (2024-11-05) 和 Streamable HTTP (2025-03-26) 两种协议；42 个内置工具涵盖文件操作、UI 主题、记忆系统、文档、项目、定时器等
- **多智能体工作区** -- 编排器模式，`TeamManager` + `AgentRunner` + `AgentTool` 生成隔离的 SubAgent；支持智能体间消息传递、共享任务看板、Agent 预设、每个 Agent 独立模型配置
- **项目工作区** -- 项目隔离会话、项目级 Memory/Knowledge 文件，以及项目作用域的 MCP 配置
...
- **多媒体能力** -- 多图视觉理解（相机拍照 + 相册选取）、通过内置 JavaScript/QuickJS 插件运行时解析 PDF/DOCX、文档生成（PDF/Excel/Word/PowerPoint）、AlarmManager 定时器（支持重复任务）；DOCX 需先上传文件，之后支持追加内容或按文本替换（不能凭空生成有效 DOCX）
...
omnichat/
	...
	│   ├── data/                        # 数据层
	│   │   ├── Entities.kt              # 17 个 Room 实体定义
	│   │   ├── Daos.kt                  # DAO 接口
	│   │   ├── AppDatabase.kt           # 数据库配置 (v61, 版本 61 迁移)
	│   │   ├── Repository.kt            # 数据仓库 (AppRepository)
	│   │   ├── ProjectContentLimits.kt  # 项目内容大小限制
	│   │   ├── ProjectFileStore.kt      # 项目私有文件存储
	│   │   └── OmnifileFormat.kt        # OMNIFILE2 + 可选项目 ZIP
	│   ├── network/
	...
	│   ├── util/
		│   │   ├── JsDocumentReader.kt      # PDF/DOCX 解析入口
		│   │   ├── JsDocumentRuntime.kt     # 内置 JavaScript/QuickJS 运行时
	│   │   ├── DocumentParseResult.kt    # 文档解析结果
	│   │   └── SessionLogExporter.kt    # 聊天日志导出
	│   └── ui/
	│       ├── screens/                 # Compose 界面（ProjectDetailScreen、ProjectKnowledgeScreen、ProjectMemoryScreen、ProjectMcpSettingsScreen）
	│       ├── viewmodel/               # ViewModel 层
	│       ├── components/              # 可复用组件（ThinkingContentParser.kt）
...
| ui_appearance | `reset_ui_to_default` | 将 UI 外观重置为默认值 |
...
| memory | `consolidate_memory` | 整理和合并记忆项 |
...
| documents | `document_read` | 读取并解析 PDF/DOCX 文档 |
...
| core | `run_workflow` | 运行工作流模板 |
| core | `export_session_log` | 导出当前会话日志 |
| project | `project_list_knowledge` | 列出项目 Knowledge 文件 |
| project | `project_read_knowledge` | 读取项目 Knowledge |
| project | `project_create_knowledge` | 创建项目 Knowledge 文件 |
| project | `project_append_knowledge` | 追加项目 Knowledge |
| project | `project_edit_knowledge` | 替换项目 Knowledge 中的文本 |
| project | `project_read_memory` | 读取项目 Memory |
| project | `project_update_memory` | 更新项目 Memory |
...
项目会话与普通会话彼此隔离。每个项目提供项目级 Memory 和 Knowledge 文件，支持 Knowledge 文本/Markdown 追加和替换、项目作用域 MCP 设置，以及上传 DOCX 文件后编辑。限制：Knowledge 总计 10 MiB、Project Memory 128 KiB、工具文本输出 100,000 字符、data URL 图片 5 MiB。
...
- **备份格式**：
  ...
  - `.omnifile` -- 当前格式为 OMNIFILE2，兼容读取 OMNIFILE1。全量 omnifile 包含 SQLite 和项目私有文件树；选择性备份只包含选择的数据库分区，不含项目 ZIP。导入时会校验 ZIP 路径和大小限制。
