...
object UiFieldRegistry {
    ...
    val colorFields = listOf(
        ...
        ColorField("accentColor", { it.accentColor }, { s, v -> s.copy(accentColor = v) },
            ...
            "强调色（思考过程中的星标等橙色点缀）"),
        ColorField("topbarSubtitleColor", { it.topbarSubtitleColor }, { s, v -> s.copy(topbarSubtitleColor = v) },
            "Top app bar subtitle color (Provider/Model info below title); default follows onSurfaceVariant",
            "顶栏副标题颜色（标题下方的 Provider/Model 信息），默认跟随 onSurfaceVariant"),
