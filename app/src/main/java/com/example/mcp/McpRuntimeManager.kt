...
class McpRuntimeManager private constructor(private val context: Context) {
    ...
    private suspend fun handleBuiltinTool(toolName: String, arguments: JSONObject): JSONObject {
        return when (toolName) {
            ...
            "reset_ui_to_default" -> {
                val db = AppDatabase.getDatabase(context)
                val repository = com.example.data.AppRepository(db)
                repository.upsertUISettings(com.example.data.UISettings())
                JSONObject().apply {
    ...
    /**
     * 构造 get_ui_capabilities 工具的返回，告知 AI：
     *   - 哪些字段可调
     *   - 每个字段的当前值与作用
     *   - 配色相关的约束（HEX 格式）和布局参数（圆角/间距）的范围
     *   - 推荐用法与最佳实践
     */
    private fun buildUiCapabilitiesResponse(current: com.example.data.UISettings): JSONObject {
