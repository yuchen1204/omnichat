...
17 sequential migrations exist (v4→v21). Versions 1–3 are handled by `fallbackToDestructiveMigrationFrom(dropAllTables = true, 1, 2, 3)` — a safety net for very old installs only. Versions 4+ always have proper incremental migrations. **Rule: 只加列/加表，绝不删数据** (only add columns/tables, never delete data).
...
- **AI-adjustable UI strings** use the `uiText("namespace.key", "默认中文")` pattern. Keys are auto-collected by the `generateUiTextKeys` Gradle task into `assets/ui_text_keys.json`. Namespaces: `topbar.*`, `sidebar.*`, `chat.*`, `models.*`, `memory.*`, `mcp.*`, `preset.*`, `export.*`, `import.*`, `tab.settings.*`, `dialog.*`, `action.*`, `workspace.*` (~130 keys total; some legacy keys use hex IDs like `chat.4c423b81`)
...
- **Workspace (multi-agent)**: Orchestrator pattern — `TeamManager` manages teammates via `TeammateContext` coroutine elements, `TaskManager` handles task CRUD with status/blocking, `MessageBus` provides inter-agent communication. See `workspace/` package.
