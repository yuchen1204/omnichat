...
**Environment note:** `gradle.properties` does NOT hardcode JDK path (set locally or via CI). Override `org.gradle.java.home` if needed.
...
- The `"settings"` view contains a **TabRow with 5 sub-tabs**: 模型配置, MCP工具, 长效记忆, Agent 预设, 数据管理
...
| `com.example.workspace` | Multi-agent system: `TeamManager`, `AgentRunner`, `AgentContext`, `TeammateContext`, `AgentLifecycle`, `AgentExecutionLoops`, `OrchestratorTools`, `TaskManager`, `MessageBus`, `WorkspaceModels` |
...
Room database `ai_chat_memory_db` (version 28) with 16 entities. See `app/src/main/java/com/example/data/Entities.kt`.
...
24 sequential migrations exist (v4→v28). Versions 1–3 are handled by `fallbackToDestructiveMigrationFrom(dropAllTables = true, 1, 2, 3)` — a safety net for very old installs only. Versions 4+ always have proper incremental migrations. **Rule: 只加列/加表，绝不删数据** (only add columns/tables, never delete data).
...
- **Built-in MCP tools (27 total)**: See `McpRuntimeManager.kt` → `builtinTools` list. Categories: core (time, ask_user, tool groups), memory (search), ui_appearance (9 tools for colors/fonts/layout), ui_text (list/set labels), files (7 CRUD ops + search/info/move), documents (create PDF/Excel/Word/PowerPoint), efficiency (timers)
...
- **Add or modify AI-adjustable UI strings**: Use the `uiText("namespace.key", "默认中文")` pattern in Compose screens. Keys are auto-collected into `assets/ui_text_keys.json`. No schema changes needed — `UiStrings` is a dynamic `Map<String, String>`. The `list_ui_texts`/`set_ui_texts` MCP tools handle runtime overrides.
